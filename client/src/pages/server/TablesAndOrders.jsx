...
// Update navigation for server interface
const navigation = [
...
// Copy ViewOrderModal, TakeOrderPanel components exactly as they are
// ... (copy the component code from the admin version)
    ...
    const handleOrderSubmit = async (data) => {
        try {
            if (editingOrder) {
                ...
                await onAddItems(formattedData);
            }
...
export default function TablesAndOrders() {
    ...
    // Keep the useEffect and data fetching
    useEffect(() => {
        fetchData();
    ...
    // ... (copy other handler functions from admin version)
    ...
    const handleAddItemsToOrder = async (orderData) => {
        try {
            ...
            // Update table status to occupied when a new order is created
            if (!orderData.orderId) {
                setTables(prev =>
                    prev.map(table =>
                        table._id === orderData.tableId ? { ...table, status: 'occupied' } : table
                    )
                );
            }
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="mt-2 text-sm text-gray-700">
                                    Active Tables: <span className="font-medium">
